...
class WDisplay {
  public:
    ...
    int sendIndexedBitmap(const uint8_t *src, unsigned width, unsigned height, uint32_t *palette) {
        if (lcd)
            return lcd->sendIndexedBitmap(src, width, height, palette);
        else
            return smart->sendIndexedBitmap(src, width, height, palette);
    }
};
...
void updateScreen(Bitmap_ img) {
    ...
    if (img) {
        ...
        // DMESG("send");
        display->sendIndexedBitmap(display->screenBuf, img->width(), img->height(), palette);
    }
    ...
    if (display->lastStatus && !display->doubleSize) {
        ...
        display->sendIndexedBitmap(display->screenBuf, img->width(), img->height(), NULL);
        display->waitForSendDone();
        display->setAddrMain();
