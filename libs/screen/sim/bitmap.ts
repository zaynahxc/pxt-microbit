namespace pxsim {
    export class RefBitmap extends RefObject {
        _width: number;
        _height: number;
        _bpp: number;
        data: Uint8Array;
...
namespace pxsim.BitmapMethods {
    ...
    export function width(img: RefBitmap) { return img._width }

    export function height(img: RefBitmap) { return img._height }

    export function isMono(img: RefBitmap) { return img._bpp == 1 }

    export function isStatic(img: RefBitmap) { return img.gcIsStatic() }

    export function revision(img: RefBitmap) { return img.revision }

    export function setPixel(img: RefBitmap, x: number, y: number, c: number) {
        img.makeWritable()
        if (img.inRange(x, y))
            img.data[img.pix(x, y)] = img.color(c)
    }

    export function getPixel(img: RefBitmap, x: number, y: number) {
        if (img.inRange(x, y))
            return img.data[img.pix(x, y)]
        return 0
    }

    export function fill(img: RefBitmap, c: number) {
        img.makeWritable()
        img.data.fill(img.color(c))
    }

    export function fillRect(img: RefBitmap, x: number, y: number, w: number, h: number, c: number) {
    ...
    export function _fillRect(img: RefBitmap, xy: number, wh: number, c: number) {
        fillRect(img, XX(xy), YY(xy), XX(wh), YY(wh), c)
    }

    export function mapRect(img: RefBitmap, x: number, y: number, w: number, h: number, c: RefBuffer) {
        if (c.data.length < 16)
            return
        img.makeWritable()
        let [x2, y2] = img.clamp(x + w - 1, y + h - 1);
    ...
    export function _mapRect(img: RefBitmap, xy: number, wh: number, c: RefBuffer) {
        mapRect(img, XX(xy), YY(xy), XX(wh), YY(wh), c)
    }

    export function equals(img: RefBitmap, other: RefBitmap) {
    ...
    export function getRows(img: RefBitmap, x: number, dst: RefBuffer) {
        x |= 0
        if (!img.inRange(x, 0))
            return
    ...
    export function setRows(img: RefBitmap, x: number, src: RefBuffer) {
        x |= 0
        if (!img.inRange(x, 0))
            return
    ...
    export function clone(img: RefBitmap) {
        let r = new RefBitmap(img._width, img._height, img._bpp)
        r.data.set(img.data)
        return r
    }

    export function flipX(img: RefBitmap) {
        img.makeWritable()
        const w = img._width
        const h = img._height
        for (let i = 0; i < h; ++i) {
    ...
    export function flipY(img: RefBitmap) {
        img.makeWritable()
        const w = img._width
        const h = img._height
        const d = img.data
    ...
    export function transposed(img: RefBitmap) {
        const w = img._width
        const h = img._height
        const d = img.data
        const r = new RefBitmap(h, w, img._bpp)
        const n = r.data
        let src = 0
    ...
    export function copyFrom(img: RefBitmap, from: RefBitmap) {
    ...
    export function scroll(img: RefBitmap, dx: number, dy: number) {
        img.makeWritable()
        dx |= 0
        dy |= 0
        if (dx != 0) {
    ...
    export function replace(img: RefBitmap, from: number, to: number) {
        to &= 0xf;
        const d = img.data
    ...
    export function doubledX(img: RefBitmap) {
        const w = img._width
        const h = img._height
        const d = img.data
        const r = new RefBitmap(w * 2, h, img._bpp)
        const n = r.data
        let dst = 0
    ...
    export function doubledY(img: RefBitmap) {
        const w = img._width
        const h = img._height
        const d = img.data
        const r = new RefBitmap(w, h * 2, img._bpp)
        const n = r.data
    ...
    export function doubled(img: RefBitmap) {
        return doubledX(doubledY(img))
    }

    function drawBitmapCore(img: RefBitmap, from: RefBitmap, x: number, y: number, clear: boolean, check: boolean) {
        x |= 0
        y |= 0
    ...
    export function drawBitmap(img: RefBitmap, from: RefBitmap, x: number, y: number) {
        drawBitmapCore(img, from, x, y, true, false)
    }

    export function drawTransparentBitmap(img: RefBitmap, from: RefBitmap, x: number, y: number) {
        drawBitmapCore(img, from, x, y, false, false)
    }

    export function overlapsWith(img: RefBitmap, other: RefBitmap, x: number, y: number) {
        return drawBitmapCore(img, other, x, y, false, true)
    }

    function drawLineLow(img: RefBitmap, x0: number, y0: number, x1: number, y1: number, c: number) {
        let dx = x1 - x0;
        let dy = y1 - y0;
        let yi = img._width;
        if (dy < 0) {
    ...
    function drawLineHigh(img: RefBitmap, x0: number, y0: number, x1: number, y1: number, c: number) {
        let dx = x1 - x0;
        let dy = y1 - y0;
        let xi = 1;
        if (dx < 0) {
    ...
    export function _drawLine(img: RefBitmap, xy: number, wh: number, c: number) {
        drawLine(img, XX(xy), YY(xy), XX(wh), YY(wh), c)
    }

    export function drawLine(img: RefBitmap, x0: number, y0: number, x1: number, y1: number, c: number) {
        x0 |= 0
        y0 |= 0
        x1 |= 0
        y1 |= 0
    ...
    export function drawIcon(img: RefBitmap, icon: RefBuffer, x: number, y: number, color: number) {
        const src: Uint8Array = icon.data
        if (!bitmap.isValidBitmap(icon))
            return
        if (src[1] != 1)
            return // only mono
    ...
    export function _drawIcon(img: RefBitmap, icon: RefBuffer, xy: number, color: number) {
        drawIcon(img, icon, XX(xy), YY(xy), color)
    }

    export function fillCircle(img: RefBitmap, cx: number, cy: number, r: number, c: number) {
        let x = r - 1;
        let y = 0;
        let dx = 1;
        let dy = 1;
    ...
    export function _fillCircle(img: RefBitmap, cxy: number, r: number, c: number) {
    ...
    export function fillTriangle(img: RefBitmap, x0: number, y0: number, x1: number, y1: number, x2: number, y2: number, c: number) {
        if (x1 < x0) {
            [x1, x0] = [x0, x1];
            [y1, y0] = [y0, y1];
        }
    ...
    export function _fillTriangle(img: RefBitmap, args: RefCollection) {
        fillTriangle(
            img,
            args.getAt(0) | 0,
            args.getAt(1) | 0,
    ...
    export function fillPolygon4(img: RefBitmap, x0: number, y0: number, x1: number, y1: number, x2: number, y2: number, x3: number, y3: number, c: number) {
        const lines: LineGenState[]= [
    ...
    export function _fillPolygon4(img: RefBitmap, args: RefCollection) {
        fillPolygon4(
            img,
            args.getAt(0) | 0,
            args.getAt(1) | 0,
    ...
    export function _blitRow(img: RefBitmap, xy: number, from: RefBitmap, xh: number) {
        blitRow(img, XX(xy), YY(xy), from, XX(xh), YY(xh))
    }

    export function blitRow(img: RefBitmap, x: number, y: number, from: RefBitmap, fromX: number, fromH: number) {
        x |= 0
        y |= 0
        fromX |= 0
        fromH |= 0
    ...
    export function _blit(img: RefBitmap, src: RefBitmap, args: RefCollection): boolean {
        return blit(img, src, args);
    }

    export function blit(dst: RefBitmap, src: RefBitmap, args: RefCollection): boolean {
...
namespace pxsim.bitmap {
    ...
    function isLegacyBitmap(buf: RefBuffer) {
    ...
    export function isValidBitmap(buf: RefBuffer) {
        if (!buf || buf.data.length < 5)
            return false;
    ...
    export function create(w: number, h: number) {
        ...
        w |= 0
        h |= 0
        return new RefBitmap(w, h, getScreenState().bpp())
    }

    export function ofBuffer(buf: RefBuffer): RefBitmap {
        const src: Uint8Array = buf.data

        let srcP = 4
        let w = 0, h = 0, bpp = 0

        if (isLegacyBitmap(buf)) {
            w = src[1]
            h = src[2]
            bpp = src[0] & 0xf;
            // console.log("using legacy bitmap")
        } else if (isValidBitmap(buf)) {
            srcP = 8
            w = bufW(src)
            h = bufH(src)
            bpp = src[1]
    ...
    export function toBuffer(img: RefBitmap): RefBuffer {
...
namespace pxsim.pxtcore {
    export function updateScreen(img: RefBitmap) {
        const state = getScreenState();
        if (state)
            state.showBitmap(img)
    }
    ...
    export function updateScreenStatusBar(img: RefBitmap) {
        const state = getScreenState();
        if (state)
