...
PXT_VTABLE(RefBitmap, ValType::Object)

void RefBitmap::destroy(RefBitmap *t) {}

void RefBitmap::print(RefBitmap *t) {
    DMESG("RefBitmap %p size=%d x %d", t, t->width(), t->height());
}

int RefBitmap::wordHeight() {
    if (bpp() == 1)
        oops(20);
    return ((height() * 4 + 31) >> 5);
}

void RefBitmap::makeWritable() {
    ++revision;
...
uint8_t RefBitmap::fillMask(color c) {
    return this->bpp() == 1 ? (c & 1) * 0xff : 0x11 * (c & 0xf);
}

bool RefBitmap::inRange(int x, int y) {
    return 0 <= x && x < width() && 0 <= y && y < height();
}

void RefBitmap::clamp(int *x, int *y) {
    *x = min(max(*x, 0), width() - 1);
    *y = min(max(*y, 0), height() - 1);
}

RefBitmap::RefBitmap(BoxedBuffer *buf) : PXT_VTABLE_INIT(RefBitmap), buffer(buf) {
    revision = 0;
    if (!buf)
        oops(21);
}
...
static inline int byteSize(int w, int h, int bpp) {
    if (bpp == 1)
        return sizeof(BitmapHeader) + ((h + 7) >> 3) * w;
    else
        return sizeof(BitmapHeader) + (((h * 4 + 31) / 32) * 4) * w;
}

Bitmap_ allocBitmap(const uint8_t *data, uint32_t sz) {
    auto buf = mkBuffer(data, sz);
    registerGCObj(buf);
    Bitmap_ r = NEW_GC(RefBitmap, buf);
    unregisterGCObj(buf);
    return r;
}

Bitmap_ mkBitmap(int width, int height, int bpp) {
    if (width < 0 || height < 0 || width > 2000 || height > 2000)
        return NULL;
    if (bpp != 1 && bpp != 4)
        return NULL;
    uint32_t sz = byteSize(width, height, bpp);
    Bitmap_ r = allocBitmap(NULL, sz);
    auto hd = r->header();
    ...
    hd->height = height;
    hd->padding = 0;
    MEMDBG("mkBitmap: %d X %d => %p", width, height, r);
    return r;
}

bool isValidBitmap(Buffer buf) {
    if (!buf || buf->length < 9)
        return false;

    auto hd = (BitmapHeader *)(buf->data);
...
bool isLegacyBitmap(Buffer buf) {
    if (!buf || buf->length < 5)
        return false;
...
Bitmap_ clone(Bitmap_ img) {
    auto r = allocBitmap(img->data(), img->length());
    MEMDBG("mkBitmapClone: %d X %d => %p", img->width(), img->height(), r);
    return r;
}
...
Bitmap_ transposed(Bitmap_ img) {
    Bitmap_ r = mkBitmap(img->height(), img->width(), img->bpp());
...
Bitmap_ doubledX(Bitmap_ img) {
    ...
    Bitmap_ r = mkBitmap(img->width() * 2, img->height(), img->bpp());
    auto src = img->pix();
    auto dst = r->pix();
    auto w = img->width();
    auto bh = img->byteHeight();
...
Bitmap_ doubledY(Bitmap_ img) {
    ...
    Bitmap_ r = mkBitmap(img->width(), img->height() * 2, img->bpp());
    auto src0 = img->pix();
    auto dst = r->pix();
...
Bitmap_ create(int width, int height) {
    Bitmap_ r = mkBitmap(width, height, IMAGE_BITS);
    if (r)
...
Buffer doubledIcon(Buffer icon) {
    if (!isValidBitmap(icon))
        return NULL;

    auto r = NEW_GC(RefBitmap, icon);
    registerGCObj(r);
